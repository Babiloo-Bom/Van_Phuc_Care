...
const handleGoogleCallback = async () => {
  ...
  // Handle error
  if (googleError) {
    const errorMessage = decodeURIComponent(googleError);
  ...
  if (googleToken) {
    try {
      ...
      // Store token first
      const authStore = useAuthStore();
      await authStore.setToken(googleToken, Date.now() + 7 * 24 * 60 * 60 * 1000);
...
const handleGoogleLogin = async () => {
  try {
    ...
    // Redirect về frontend callback page (giống elearning và crm)
    const redirectUri = `${baseFrontend}/auth/google/callback`;
    const frontendUrl = baseFrontend;

    // Google OAuth dùng /api/a (admin endpoint)
