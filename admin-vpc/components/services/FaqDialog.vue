<template>
    <a-modal
        v-model="visible"
        destroy-on-close
        :title="_isEmpty(faq) ? 'Thêm câu hỏi' : 'Chỉnh sửa câu hỏi'"
    >
        <a-form-model
            ref="form"
            :model="form"
        ...
        >
            <div class="grid grid-cols-1 gap-4">
                ...
                <Editor v-model="form.content" title="Nội dung" />
            </div>
        </a-form-model>
...
<script>
    ...
    export default {
        ...
        watch: {
            faq() {
                this.form = this.faq ? _cloneDeep(this.faq) : defaultForm;
            },
        },
        ...
        methods: {
            ...
            async create(form) {
                try {
                    await this.$api.faqs.create({ ...form, serviceId: this.service._id });
            ...
            async update(form) {
                try {
                    await this.$api.faqs.update(form._id, { ...form, serviceId: this.service._id });
