    ...
    <div>
        ...
        <div v-if="data?.length" class=" bg-white rounded-sm">
            ...
            >
                ...
                >
                    <template #default="record">
                        <span v-if="record.descriptions" class="truncate-2" v-html="record.descriptions" />
                        <span v-else>--</span>
                    </template>
                </a-table-column>
                <a-table-column
                    ...
                    fixed="right"
                >
                    <template #default="process, record, index">
                        <a-button
                            type="primary"
                            shape="circle"
                            @click="() => {
                                progressSelected = process,
                                $refs.ProgressDialog.open(process, index)
                            }"
                        >
                            <svg
                                viewBox="0 0 24 24"
...
<script>
    ...
    import ProgressDialog from '@/components/services/ProgressDialog.vue';
    ...
    export default {
        ...
        methods: {
            async updateStatus(process) {
                this.progressSelected = process;
                try {
                    this.loadingTable = true;
                    this.$api.data.update(this.progressSelected._id, { status: this.statusWitch ? 'active' : 'inactive' });
            ...
            onChange(process) {
                this.statusWitch = process;
            },
            async confirmDelete() {
                try {
                    this.loadingTable = true;
                    await this.$api.services.update(this.service._id, { progress: this.service.progress.filter((e) => e.title !== this.progressSelected.title) });
