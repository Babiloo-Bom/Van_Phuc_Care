        ...
        >
            <div class="grid grid-cols-1 mb-[2px]">
                ...
                <a-radio-group v-model="form.type" name="radioGroup">
                    ...
                    <a-radio value="selected" :disabled="!cartSelected.length" :style="radioStyle">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('orders/carts', ['carts', 'pagination', 'cartSelected']),
            presetRanges() {
                return {
        ...
        methods: {
            ...
            async fetchCarts(params) {
                const { data: { carts } } = await this.$api.carts.getAll({ ...params, limit: 1000 });
                return carts;
            },

            async handleConfirm() {
                if (this.form.type === 'current_page') {
                    this.downloadExcel(this.carts);
                } else if (this.form.type === 'selected') {
                    this.downloadExcel(this.carts && this.carts?.filter((e) => this.cartSelected.includes(e._id)));
                } else {
                    try {
                        this.loading = true;
                        const carts = await this.fetchCarts({ ...this.query });
                        this.downloadExcel(carts);
                        this.loading = false;
                        this.close();
                    } catch (error) {
            ...
            downloadExcel(carts) {
                ...
                worksheet.addRows(carts?.map((record) => ({
                    _id: record._id,
                    products: record.items?.length,
